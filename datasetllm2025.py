...
class datasetllm(Dataset):
    def __init__(self, dataset):
        ...
        self.prompts, self.img_set, self.mos1, self.text_instruct, self.text_out = self.make_data()
    ...
    def make_data(self):
        ...
        for data_item in self.data:
            item = json.loads(data_item)
            
            item['q1'] = ""
            item['a1'] = ""
            img_path = os.path.join(opts.imgpath, item['image'])
            
            # pil_image = Image.open(img_path)
            # image = self.preprocess(pil_image)
            img_set.append(img_path)
            prompts.append(item['conversations'][0]['value'].split('rompt:')[1])
            mos1.append(item['mos'])
            # mos2.append(item['moz2'])
            # mos3.append(item['moz3'])
            q1.append(item['conversations'][0]['value'])
            a1.append(item['conversations'][1]['value'])
 
            bar.update(1)

        return prompts, img_set, mos1, q1, a1 
